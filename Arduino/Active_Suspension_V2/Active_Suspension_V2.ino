...
//function that converts position from number of counts to height; using counts per 
...
//function that??
double countsToHz(long counts, double cpr, double duration)
{
  double freq = abs(counts);
  freq /= duration;
  freq /= cpr;
  freq *= 1000;
  return freq;
...
void loop() {
  ...
  double motorSpeed = findSpeed(motorPosInCounts, 3415.92, timePassed);
